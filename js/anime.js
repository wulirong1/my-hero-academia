document.addEventListener('DOMContentLoaded', () => {
 ...
 if (container) {
    ...
    function closeAllAnime() {
        ...
        gsap.to(portal, { opacity: 0, scale: 0.8, duration: 0.2, onComplete: () => gsap.set(portal, { display: 'none' }) });
    ...
    container.addEventListener('click', (e) => {
        ...
        if (item) { e.stopPropagation(); item.classList.contains('active') ? closeAllAnime() : openAnimeCard(item); }
    });
    ...
    document.addEventListener('visibilitychange', () => {
        const iframe = portal.querySelector('iframe');
        if (document.hidden && window.getComputedStyle(portal).display === 'flex') {
            const currentSrc = iframe.src;
            if (currentSrc) { iframe.setAttribute('data-last-src', currentSrc); iframe.src = ""; }
        } else if (!document.hidden && window.getComputedStyle(portal).display === 'flex') {
